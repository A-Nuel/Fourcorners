...
/**
 * Client-side persistence policy:
 * - Compare list: non-sensitive UI preference → localStorage OK
 * - Jobs / sessions: ephemeral sessionStorage only — never seed fake on-chain proofs
 * - No private keys, no seed phrases, no session private material ever stored
 */
...
export function saveJob(job: HireJob): void {
  ...
  try {
    ...
    // Cap history to reduce client bloat
    const trimmed = current.slice(0, 25);
    sessionStorage.setItem(JOBS_KEY, JSON.stringify(trimmed));
...
export function saveSession(session: AltanaSession): void {
  if (!canUseBrowserStorage()) return;
  try {
    // Strip any accidental sensitive fields — only public session metadata
